...
impl<T: Transport> Future for ResolveGasPriceFuture<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
        ...
        match this.state.project() {
            ...
            ResolveGasPriceState::Estimating(gas_price_estimate, factor) => {
                gas_price_estimate.poll(cx).map(|gas_price_estimate| {
                    let gas_price_estimate = gas_price_estimate?;
                    if let Some(factor) = factor {
                        Ok(apply_gas_price_factor(gas_price_estimate, *factor))
                    } else {
                        Ok(gas_price_estimate)
                    }
                })
            }
        }
...
/// Apply
fn apply_gas_price_factor(gas_price: U256, factor: f64) -> U256 {
