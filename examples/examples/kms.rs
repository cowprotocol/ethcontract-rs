use ethcontract::{aws_config, prelude::*, transaction::{TransactionBuilder, kms, Account}};
use std::env;
...
async fn main() {
    ...
    let account = kms::Account::new((&config).into(), &env::var("KMS_KEY_ID").expect("KMS_KEY_ID not set"))
        .await
        .unwrap();
    ...
    println!("Sending transaction to self: {:?}", account.public_address());
